...
## 当前状态
- **评审进行中（2026-09-26）**：需求编号已统一（新需求 6-11 + 军饷需求 12）；煤/桑链冻结在骨架外；油井=城外采集供军事；军饷要做（粮拆口粮/军饷两种）。**土三档读法待用户澄清**（"一三先不涉及"=砍掉砂土黄土只留灰土？还是选"土=一种库存"方案？）——确认前不动土条目与 engine
- **v2 骨架数值已落地（`dea3871`，2026-09-26，未评审产物）**：方案 B（分层战斗+90天时间线）可仿真。新文件：`TUNING-v2.js`/`engine-v2.js`/`simulate-v2.js`/`数值提案-战斗与城防-v2.md`；旧三件套（TUNING/engine/simulate v0.9）冻结不删。五指标全 PASS：正统双流 100%（均衡流灰土线贴脸 vs 墙塔流黄土线从容），经济流 2.7%「富而破」，裸墙 0%
- **v2 关键定稿（部分待评审重确认）**：土三档 600/1400/2400（灰土 1400 是模拟校准产物——初稿 1200 让灰土线不可行）；箭塔削10骑(2兵)/弩炮削25骑(3兵)，单波上限40%；兵=征召制；石=高级建材退出修墙。**军饷拍板推翻了"粮即饷"，engine-v2 的口粮模型要改**
- **v2 遗留待拍板**：破防惩罚触发条件——「摸墙即罚」让修墙无博弈价值（均衡流断粮22天粮1斛），候选改「墙血<50%才触发」。机制变更，等用户拍板再动 engine
- **下一入口**：①土三档读法澄清 ②军饷机制设计（需求12：来源/生产/欠饷后果）③破防机制拍板 ④demo 骨架重写（90天时间线 UI，真人手感验证）
