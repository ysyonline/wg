// TUNING.js —— 长城守城 · 全游戏唯一数字来源 v0.9-paper（模拟校准版）
// 纪律：
...
const TUNING = {
  ...
  // —— 人口效率权重 [v0.9] ——
  // 设计意图：堵"塞满人无限产木"漏洞——伐木/采石/采矿无人数上限，招1人20粮产8木吃2粮，
  // 不到两回合回本，无脑招人是模拟里的隐性最优解。权重让"第N个人还值不值20粮"变成真决策。
  // 只乘工产不乘田产：田已有 WORK_CAP=4人/块 的物理上限，不重复罚。
  // 口粮照吃满额：超编者"吃满额、干半活"，刹车感来自看得见的产出缩水。
  // 曲线：≤SMALL 全员0.9（精干加成，翻盘补偿）→ SMALL~BASE 1.0（标准）→ >BASE 每超1人+0.04（平滑斜坡，无跳变）。
  // [PLACEHOLDER·待模拟] 三条数字（20/45/0.04）首轮跑完压榨流探针后校准。
  POP_EFF: {
    SMALL: 20,      // 精干线：≤20人 全员权重0.9（+11%产出）。开局30人进不去——只有遭内乱减员后触发，是给翻盘者的补偿
    BASE: 45,       // 标准线：≤45人 权重1.0。开局30+15余量：前两回合无脑招人无感，第3回合起边际开始下滑
    SMALL_W: 0.9,   // 精干权重：不再更低——防止"故意减员刷效率"变成套路
    SLOPE: 0.04,    // 超编斜率：每超1人权重+0.04。第1个超编者几乎无感(×0.998)，第10个产5.7木，第20个4.4
    MAX_W: 2.5,     // 权重上限：超编者永远有微弱正产出(3.2木/人)，钝刀不死刑——死刑仍归断粮-25民意
  },
